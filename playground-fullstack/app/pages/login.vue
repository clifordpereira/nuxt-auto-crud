    ...
    <div class="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-20 xl:px-24 bg-gray-50 dark:bg-gray-950">
      <div class="mx-auto w-full max-w-sm lg:w-96">
        <div class="lg:hidden mb-8 text-center">
          <UIcon name="i-heroicons-command-line" class="w-12 h-12 text-primary-500 mx-auto" />
          <h2 class="mt-4 text-2xl font-bold text-gray-900 dark:text-white">Nuxt Auto CRUD</h2>
        </div>
        ...
        <div class="mt-10">
          <form class="space-y-8" @submit.prevent="onSubmit">
            <UFormGroup label="Email address" name="email">
              <UInput v-model="state.email" type="email" autocomplete="email" required icon="i-heroicons-envelope" size="lg" />
            </UFormGroup>

            <UFormGroup label="Password" name="password">
              <UInput v-model="state.password" type="password" autocomplete="current-password" required icon="i-heroicons-lock-closed" size="lg" />
            </UFormGroup>

            <div>
              <UButton type="submit" block :loading="loading" size="lg" icon="i-heroicons-arrow-right-on-rectangle">
                Sign in
              </UButton>
            </div>
          </form>

...
describe("modelMapper.ts", () => {
  ...
  it("generates partial schemas for patch operations", () => {
    ...
    const schema = mapper.getZodSchema(
      "users",
      "patch",
  ...
  it("formats resource results (single and array)", () => {
    ...
    expect((resArray as Record<string, unknown>[])[0]).not.toHaveProperty(
      "password",
    );
  });
  ...
  it("generates a complete schema definition", () => {
    const schema = mapper.getSchemaDefinition("users");

    expect(schema.resource).toBe("users");
    expect(schema.labelField).toBe("name");

    const fieldNames = schema.fields.map((f) => f.name);
    expect(fieldNames).toContain("email");
    expect(fieldNames).toContain("id");
    expect(fieldNames).not.toContain("password"); // Hidden

    const idField = schema.fields.find((f) => f.name === "id");
    expect(idField?.isReadOnly).toBe(true); // Protected
    expect(idField?.type).toBe("number");

    const lastLoginField = schema.fields.find((f) => f.name === "lastLogin");
    expect(lastLoginField?.type).toBe("date");
  });

  it("includes relations in schema definition", () => {
    const schema = mapper.getSchemaDefinition("posts");
    const relationField = schema.fields.find((f) => f.references === "users");

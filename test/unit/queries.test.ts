...
describe('NAC Core Queries - Consolidated Suite', () => {
  ...
  describe('getRows()', () => {
    it('applies list permission logic (status OR owner)', async () => {
      ...
      await getRows(posts as unknown as TableWithId, { userId: '1', permissions: ['list'] })
      expect(db.where).toHaveBeenCalled()
    })

    it('applies list_own permission logic strictly', async () => {
      vi.mocked(db.all).mockResolvedValue([])
      await getRows(posts as unknown as TableWithId, { userId: '1', permissions: ['list_own'] })
      expect(db.where).toHaveBeenCalled()
    })

    it('bypasses filters for admin (no permissions provided)', async () => {
      vi.mocked(db.all).mockResolvedValue([])
      await getRows(posts as unknown as TableWithId, {})
    ...
    it('handles tables missing status/owner columns gracefully', async () => {
      ...
      await getRows(users as unknown as TableWithId, { userId: '1', permissions: ['list'] })
      expect(db.where).not.toHaveBeenCalled()
    })
  ...
  describe('getRow()', () => {
    it('returns sanitized record from context cache immediately', async () => {
      const result = await getRow(users as unknown as TableWithId, '1', {
    ...
    it('throws RecordNotFoundError when ID is missing', async () => {
      ...
      await expect(getRow(posts as unknown as TableWithId, '999')).rejects.toThrow(RecordNotFoundError)
    })

    it('coerces string ID to number for query safety', async () => {
      vi.mocked(db.get).mockResolvedValue({ id: 5 })
      await getRow(posts as unknown as TableWithId, '5')
      expect(db.where).toHaveBeenCalled()
    })
    ...
    it('sanitizes cached record from context using selectableFields', async () => {
      ...
      // Pass '1' instead of 1
      const result = await getRow(users as unknown as TableWithId, '1', context)
    ...
    it('sanitizes DB result by passing selectableFields to db.select', async () => {
      await getRow(users as unknown as TableWithId, '1')
  ...
  describe('createRow()', () => {
    it('injects audit trail and refreshes updatedAt', async () => {
      ...
      await createRow(posts as unknown as TableWithId, { title: 'Test' }, { userId: '10' })
    ...
    it('respects runtimeConfig ownerKey override', async () => {
      ...
      await createRow(mockTable as unknown as TableWithId, {}, { userId: '1' })
    ...
    it('skips audit fields if columns missing from schema', async () => {
      ...
      await createRow({ id: {} } as unknown as TableWithId, { title: 'No Audit' }, { userId: '1' })
      const payload = vi.mocked(db.values).mock.calls[0]![0] as Record<string, unknown>
    ...
    it('throws InsertionFailedError on empty DB response', async () => {
      vi.mocked(db.get).mockResolvedValue(null)
      await expect(createRow(posts as unknown as TableWithId, {})).rejects.toThrow(InsertionFailedError)
    })

    it('maintains input object immutability', async () => {
      const input = { title: 'Original' }
      vi.mocked(db.get).mockResolvedValue({ id: 1 })
      await createRow(posts as unknown as TableWithId, input, { userId: '1' })
    ...
    it('enforces selectableFields in the returning clause', async () => {
      ...
      await createRow(users as unknown as TableWithId, { name: 'New User' })
  ...
  describe('updateRow()', () => {
    it('refreshes updatedAt and updatedBy specifically', async () => {
      ...
      await updateRow(posts as unknown as TableWithId, '1', { title: 'Edit' }, { userId: '50' })
    ...
    it('targets correct row via numeric ID', async () => {
      vi.mocked(db.returning).mockResolvedValue([{ id: 1 }])
      await updateRow(posts as unknown as TableWithId, '200', {})
      expect(db.where).toHaveBeenCalled()
    })

    it('throws UpdateFailedError when update fails', async () => {
      vi.mocked(db.returning).mockResolvedValue([])
      await expect(updateRow(posts as unknown as TableWithId, '1', {})).rejects.toThrow(UpdateFailedError)
    })

    it('enforces selectableFields filter on return', async () => {
      vi.mocked(db.returning).mockResolvedValue([{ id: 1 }])
      await updateRow(users as unknown as TableWithId, '1', {})
  ...
  describe('deleteRow()', () => {
    it('returns the deleted record data', async () => {
      ...
      const res = await deleteRow(posts as unknown as TableWithId, '1')
      expect(res.title).toBe('Deleted')
    })

    it('throws DeletionFailedError for invalid IDs', async () => {
      vi.mocked(db.get).mockResolvedValue(null)
      await expect(deleteRow(posts as unknown as TableWithId, '404')).rejects.toThrow(DeletionFailedError)
    })
  })
})

...
## 🛠️ Metadata Structure (Agentic Discovery)

The `_meta` response provides high-fidelity signals for tool calling:

* **`labelField`**: The key to be used as a human-readable identifier (e.g., `name` for Roles, `email` for Subscribers).
* **`isRelation`**: Boolean flag indicating foreign key relationships.
* **`references`**: String pointing to the target resource (e.g., `roleId` references `roles`).

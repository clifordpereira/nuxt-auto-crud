...
However, your frontend application needs to *know* what it is allowed to do, even for a guest user who hasn't logged in yet. This is where the `/api/_nac/public-permissions` endpoint comes in.
...
export const abilityLogic = async (user, model, action) => {
  ...
  if (!publicPermissionsCache) {
    publicPermissionsCache = await $fetch('/api/_nac/public-permissions')
      .catch(() => ({}))
  }

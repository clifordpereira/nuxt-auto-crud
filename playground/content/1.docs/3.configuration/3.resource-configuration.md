...
> **AI Snapshot:** Resource configuration in the `nuxt-auto-crud stack` manages the serialization boundary between your Drizzle schema and the JSON API. While the [Core Engine] enforces data protection via `getZodSchema`, the [Template Implementation] configures visibility tiers for Guests and Authenticated users.
